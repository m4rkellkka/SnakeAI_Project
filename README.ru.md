...
- **Tauri-приложение** — автономный GUI (`app/`) с живыми графиками обучения, пятью панелями (Train / Play / Benchmark / Models / Settings), рендерингом игры на Canvas и управлением процессами; Python не нужен конечному пользователю
...
- **Каналы 0–5 (эгоцентричные, повёрнутые):** голова, тело, еда, карта опасности (стены + тело), полнота поля, история посещений
  ...
  - Канал 5 (история посещений): нормализованный счётчик посещений клетки с момента последнего яблока (максимум 4) — помогает детектировать петли
...
- **DAgger:** 0–70% шагов используют действие сети (разогрев за 100k шагов), выявляя распределительный сдвиг
...
Полнофункциональная тёмная панель управления с пятью панелями:

- **Train** — запуск/остановка обучения, живые графики score + eval, цветной лог
- **Play** — игра прямо в приложении на Canvas с двумя режимами: **You** (чистый JS-движок, WASD/стрелки) и **Watch AI** (стриминг из `src/stream_game.py` для Pretrained / Best checkpoint / Teacher); скорость меняется на лету в обоих режимах
...
- **Settings** — конфигурация приложения и статус окружения
...
| `src/stream_game.py` | Безголовый стример игры: запускает чекпойнт или учителя, выдаёт JSON-кадры — питает Watch AI в панели Play |
| `src/state_encoding.py` | Общая 10-канальная кодировка состояния для `train_ai.py` (BC) и `train_rl.py` (RL) |
| `src/train_rl.py` | RL-тренер (Double DQN с Dueling head) — метод обучения с подкреплением, без меток учителя |
...
- `get_state(game)` — Строит 10-канальное состояние с поворотом и компасом
...
- Вход: 10 каналов, выход: 3 logits (должны совпадать с пространством действий)
...
Базовая версия с поведенческим клонированием — завершена:

- ✅ **Tauri-приложение** — тёмная GUI-панель с живыми графиками обучения и управлением процессами
...
Новые парадигмы обучения помимо имитационного, плюс обновлённое десктоп-приложение:

- ✅ **Обучение с подкреплением (RL)** — Double DQN с Dueling head (`src/train_rl.py`), обучение через reward-сигнал без меток учителя; общая кодировка состояния с BC через `src/state_encoding.py`
- ✅ **Панель Play в приложении** — игра на Canvas с режимами You (чистый JS-движок) и Watch AI (JSON-стример), заменила старую Watch-панель с Pygame-окном
- ✅ **JSON-стример игры** — безголовый `src/stream_game.py` передаёт кадры как JSON для фронтенда Tauri
- ✅ **Панель Settings** — конфигурация приложения и статус окружения
