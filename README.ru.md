...
#### Учитель
`teacher.py`: Алгоритм на цикле Гамильтона, который:
...
| Файл | Назначение |
|------|-----------|
| `snake_game.py` | Двигатель игры (`SnakeGameAI`), константы сетки, цикл Гамильтона |
| `teacher.py` | Совершенный алгоритм цикла Гамильтона с сокращениями; демо |
| `train_ai.py` | Сеть (`SnakeNet`), буфер воспроизведения, тренер, агент, цикл обучения/оценки |
...
**Гиперпараметры** (отредактируйте `train_ai.py`):
...
**Недостаточно памяти?**
- Уменьшите `MAX_MEMORY` (ёмкость буфера) в `train_ai.py`
