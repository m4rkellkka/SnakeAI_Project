...
Запланированные следующие шаги по направлениям. Направления не строго последовательны — у некоторых есть зависимости, отмеченные ниже.

#### Направление A — Качество модели (текущий BC + DAgger pipeline)

- Прогнать sweep-запуски по `DAGGER_PROB_MAX` (0.7), `CURRICULUM_PROB`, `LR` через `--run-name` + `tools/benchmark.py` и сравнить результаты
- Эксперименты с архитектурой `SnakeNet` — residual blocks, более глубокие свёртки, другие размеры FC
- Более долгие тренировки сверх текущего чекпойнта на 350 играх

#### Направление B — Новые подходы к обучению

- **Reward-слой для RL** — отдельная обёртка, которая считает reward из `(game_over, score)` без изменения существующего контракта `play_step` (BC по дизайну остаётся без reward)
- **`src/train_rl.py`** — DQN/PPO с нуля (без учителя), на той же 9-канальной state-репрезентации
- **Multi-snake окружение** — обобщить `SnakeGameAI` на N змеек: столкновения змейка-змейка, общая еда, каналы состояния с учётом соперника
- **Self-play / AI vs AI** — агенты, обучающиеся друг против друга на multi-snake окружении
- **Ensemble / сравнение моделей** — прогон BC, RL и self-play чекпойнтов друг против друга через benchmark harness

#### Направление C — Игровые режимы

- **Человек vs AI** — расширить `play_manual.py` на multi-snake окружение, одна змейка управляется человеком
- **Другие поля / препятствия** — обобщить `GRID_SIZE`, добавить клетки-препятствия. Учитель на цикле Гамильтона не работает с препятствиями, поэтому этот пункт зависит от направления B (RL-агент или новый "несовершенный" учитель на BFS/A*)
- **Вкладка "Турнир/лидерборд" в `launcher.py`** — выбрать чекпойнты/режимы, прогнать через benchmark harness, показать таблицу результатов

#### Полировка для презентации

- Бейджи статуса (версия Python, лицензия, фреймворк)
