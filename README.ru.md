...
- **Benchmark harness** — скрипт, который прогоняет N игр для одного или нескольких чекпойнтов и выдаёт полное распределение score (не только avg/max). Более точный сигнал для сравнения гиперпараметров/архитектур и основа для турнира/лидерборда ниже.

#### Направление A — Качество модели (текущий BC + DAgger pipeline)

- Подбор гиперпараметров: `DAGGER_PROB_MAX` (0.7), `CURRICULUM_PROB`, `LR` — через benchmark harness
